...
## Lessons Learned (Critical — Read Before Generating)

⚠️ **These are hard-won lessons from real failures. Read before generating any file.**

### 1. Generation MUST use a script, never hand-write JSON

Use Python or Node.js to generate the `.excalidraw` file. JSON does not support comments, and hand-writing 100+ elements inevitably introduces syntax errors.

### 2. fontFamily must be 1, 2, or 3

The schema only supports `fontFamily: 1` (Virgil), `2` (Helvetica), or `3` (Cascadia). **Do NOT use `5`** — it is not in the schema and causes "invalid file" errors.

### 3. gridSize must be a number, not null

```json
"appState": {
  "viewBackgroundColor": "#ffffff",
  "gridSize": 20   // ✅ Must be a number
}
```

### 4. Every element's `index` field must be globally unique

Use a monotonically incrementing counter for ALL indices. Do NOT reuse index values (e.g., don't give all arrows the same index `"m0"`).

### 5. roundness must be set correctly per element type

| Element type | roundness value |
|---|---|
| `rectangle` | `{"type": 3}` |
| `arrow` | `{"type": 2}` |
| `text` | `null` |

### 6. NEVER use the `text` property on rectangles for labels

Rectangle embedded text (`"text": "App.tsx"`) does **NOT render** when imported via JSON. Always use a separate `type: "text"` element positioned over the rectangle.

### 7. NEVER use `containerId` / `boundElements` bindings

Binding text to containers via `containerId` and `boundElements` causes **"Unable to load initial data"** on import. Position text elements independently using arithmetic.

### 8. autoResize must be false for labels inside boxes

```json
"autoResize": false  // ✅ Prevents text overflow
```
With `autoResize: true`, text elements expand beyond their specified bounds.

### 9. Text centering requires manual calculation

Do NOT rely on `verticalAlign: "middle"` — it does not work reliably with `autoResize: false`.

```python
text_h = fontSize + 4          # enough for one line of text
tx = rect_x                     # same x as rectangle
ty = rect_y + (rect_h - text_h) // 2  # vertically centered
text_element = {
    "x": tx, "y": ty,
    "width": rect_w,
    "height": text_h,
    "textAlign": "center",
    "verticalAlign": "middle",   # works within the smaller text box
    "autoResize": false
}
```

### 10. Always validate before delivering

Run these checks before providing the file:

```python
from collections import Counter

data = json.load(open("file.excalidraw"))
elements = data["elements"]

# Check IDs
ids = [el["id"] for el in elements]
assert not [k for k,v in Counter(ids).items() if v>1], "Duplicate IDs"

# Check indices
indices = [el["index"] for el in elements]
assert not [k for k,v in Counter(indices).items() if v>1], "Duplicate indices"

# Check roundness
for el in elements:
    t = el["type"]
    r = el.get("roundness")
    if t == "rectangle": assert r == {"type": 3}, f"{el['id']} roundness"
    if t == "arrow":     assert r == {"type": 2}, f"{el['id']} roundness"
    if t == "text":      assert r is None,        f"{el['id']} roundness"

# Check no container bindings
for el in elements:
    assert el.get("boundElements") is None, f"{el['id']} has boundElements"
    assert el.get("containerId") is None,  f"{el['id']} has containerId"

# Check fontFamily (text elements only)
for el in elements:
    if "fontFamily" in el:
        assert el["fontFamily"] in (1, 2, 3), f"{el['id']} fontFamily={el['fontFamily']}"
```

### 11. Test with a minimal file first

Before generating a full diagram (100+ elements), create a minimal test file with 1 rectangle + 1 text element and verify it opens in https://excalidraw.com.

### Quick Reference: Correct Element Patterns

```python
# ✅ Rectangle (no text property)
{
    "type": "rectangle",
    "roundness": {"type": 3},
    # NO "text" field
...
# ✅ Text (standalone, positioned manually)
{
    "type": "text",
    "roundness": None,
    "fontFamily": 1,
